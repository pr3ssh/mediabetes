...
class Informe extends CI_Controller {
    ...
    /*public function make()
    {
    	...
    	elseif (($this->input->post('fecha_inicio') == null) && ($this->input->post('fecha_fin') != null)) {
			...
			foreach ($medidas as $medida) {
				...
				$datos_medida[] = array('valor' => $medida->valor,'fecha' => $medida->fecha,'hora' => $medida->hora,'tipo' => $nombre_tipo);
    ...
    public function make()
    {
    	...
    	elseif (($this->input->post('fecha_inicio') == null) && ($this->input->post('fecha_fin') != null)) {
    		...
    		$medidas = $this->Medida_model->get_medidas_between_dates($date_ini,$date_end);
    		$medidas_contador = $this->Medida_model->get_count_medidas_between_dates($date_ini,$date_end);
    		$total_dias = count($medidas_contador);
    		$i = 0; $indice = 0; $j = 0;
    		while ($i < $total_dias) {
    			$medidas_del_dia = $medidas_contador[$i]->total;
    			$j = 0;
    			$d = date_parse($medidas_contador[$i]->fecha);
    			$value_d = $d['day'].'/'.$d['month'].'/'.$d['year'];
    			$dia = array ('dia' => $value_d, '1' => '', '2' => '', '3' => '', '4' => '', '5' => '', '6' => '', '7' => '');
    			while ($j < $medidas_del_dia) {
    				$h = intval($medidas[$indice]->tipo);
    				$dia[$h] = $medidas[$indice]->valor;
    				$j++; $indice++;
    			}
    			$datos_medida[] = $dia;
    			$i++;
            ...
            $col_names = array(
                ...
                '2' => 'Despues del desayuno',
                '3' => 'Antes del almuerzo',
                '4' => 'Despues del almuerzo',
                '5' => 'Antes de la cena',
                '6' => 'Despues de la cena',
                '7' => 'Madrugada'
            );
